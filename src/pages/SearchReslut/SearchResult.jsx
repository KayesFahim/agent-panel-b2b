...
import { Box, Grid, Button, Typography, Drawer, IconButton } from "@mui/material";
...
const SearchResult = () => {
  ...
  const [selectedDuration, setSelectedDuration] = useState("all"); // 'all'|'short'|'medium'|'long' (transit) or flight duration
  ...
  useEffect(() => {
    ...
    const url = `${import.meta.env.REACT_APP_API_URL}/agent/flight/search`;
    const body = {
      adultcount: adultCount,
      childcount: childCount,
      infantcount: infant,
    ...
    const fetchData = async () => {
      try {
        ...
        } else if (err.message.includes("No Flights Found")) {
  ...
  const sortedFilterData = useMemo(() => {
    ...
    if (sortBy === "price_asc") {
      sorted.sort((a, b) => (a.NetFare || a.GrossFare || 0) - (b.NetFare || b.GrossFare || 0));
    } else if (sortBy === "price_desc") {
      sorted.sort((a, b) => (b.NetFare || b.GrossFare || 0) - (a.NetFare || a.GrossFare || 0));
    } else if (sortBy === "duration_asc") {
      sorted.sort((a, b) => {
        const durA = a.AllLegsInfo?.reduce((sum, leg) => sum + (leg.Duration || 0), 0) || 0;
        const durB = b.AllLegsInfo?.reduce((sum, leg) => sum + (leg.Duration || 0), 0) || 0;
        return durA - durB;
      });
    } else if (sortBy === "dep_asc") {
      sorted.sort((a, b) => {
        const depA = a.AllLegsInfo?.[0]?.Segments?.[0]?.DepTime || "";
        const depB = b.AllLegsInfo?.[0]?.Segments?.[0]?.DepTime || "";
  ...
  const groupedFilterData = useMemo(() => {
    ...
    return Array.from(groups.values()).map(group => {
      // Respect the active sort order by keeping group[0] as the primary according to sortedFilterData
      return {
        primary: group[0],
        options: group
      };
              ...
              >
                <Grid xs={12} item container justifyContent="space-between">
                  <Grid item>
                    ...
                    >
                      Total passenger {adultCount > 0 && `Adult(${adultCount})`}
                ...
                <Grid item xs={12} sm={12} md={12} lg={12}>
                  {isLoaded ? (
                    <Box>
                      ...
                      {/* Premium Result Counter & Sorting Panel */}
                      <Box
                        display="flex"
                      ...
                      >
                        <Typography variant="body2" fontWeight={600} color="var(--neutral-800)">
                          Showing <span style={{ color: "var(--primary-color)" }}>{filterData?.length}</span> flights found
                        </Typography>

                        <Box display="flex" alignItems="center" gap={1.5}>
                          <Typography variant="caption" fontWeight={650} color="var(--neutral-500)" sx={{ textTransform: "uppercase", letterSpacing: "0.5px" }}>
                            Sort By:
                          </Typography>
                          <select
                            value={sortBy}
                          ...
                          {pageData.map((data, index) => {
                            return (
                              <Box key={index}>
                                ...
                                <SingleFlight
                                  key={index}
                                  ischeapest={index === 0 && pageIndex === 0}
